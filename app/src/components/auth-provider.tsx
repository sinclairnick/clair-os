...
export function AuthProvider({ children }: { children: ReactNode }) {
	...
	const { lastFamilyId, setLastFamilyId } = useAppStore();
	...
	const isFamiliesLoading = familiesQueryResult.isLoading;

	// Current family selection from store
	const storedFamilyId = lastFamilyId;
	...
	// Effect to sync current family to store when families load
	useEffect(() => {
		if (families.length > 0 && !storedFamilyId) {
			setLastFamilyId(families[0].id);
		}
	}, [families, storedFamilyId, setLastFamilyId]);

	const globalIsLoading = isLoading || (isAuthenticated && isFamiliesLoading);
